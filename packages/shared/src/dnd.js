;(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    : ((global = typeof globalThis !== 'undefined' ? globalThis : global || self),
      factory((global.ReactBeautifulDnd = {}), global.React, global.ReactDOM, global.shim))
})(this, function (exports, React, ReactDOM, shim) {
  'use strict'
...
`
  ...
  function useMemoOne(getResult, inputs) {
    ...
    var useCache =
      isFirstRun.current ||
      Boolean(
        inputs && committed.current.inputs && areInputsEqual$1(inputs, committed.current.inputs),
      )
    var cache = useCache
      ? committed.current
      : {
  ...
  var parse = function parse(raw) {
    ...
    !!isNaN(result)
      ? invariant(false, 'Could not parse value [raw: ' + raw + ', without suffix: ' + value + ']')
      : void 0
    return result
  }
  ...
  var moveToNextIndex = (_ref) => {
    ...
    !wasAt
      ? invariant$1(false, 'Cannot move in direction without previous impact location')
      : void 0
  ...
  const addPlaceholder = (droppable, draggable, draggables) => {
    ...
    !!isHomeOf(draggable, droppable)
      ? invariant$1(false, 'Should not add placeholder space to home list')
      : void 0
    !!droppable.subject.withPlaceholder
      ? invariant$1(false, 'Cannot add placeholder size to a subject when it already has one')
      : void 0
  ...
  const removePlaceholder = (droppable) => {
    ...
    !added
      ? invariant$1(false, 'Cannot remove placeholder form subject when there was none')
      : void 0
    const frame = droppable.frame
    if (!frame) {
      const subject = getSubject({
  ...
  var getDragImpact = (_ref) => {
    let { pageOffset, draggable, draggables, droppables, previousImpact, viewport, afterCritical } =
      _ref
  ...
  var reducer = function (state, action) {
    ...
    if (action.type === 'INITIAL_PUBLISH') {
      !(state.phase === 'IDLE')
        ? invariant$1(false, 'INITIAL_PUBLISH must come after a IDLE phase')
        : void 0
    ...
    if (action.type === 'COLLECTION_STARTING') {
      ...
      !(state.phase === 'DRAGGING')
        ? invariant$1(false, `Collection cannot start from phase ${state.phase}`)
        : void 0
      const result = {
        ...state,
        phase: 'COLLECTING',
    ...
    if (action.type === 'PUBLISH_WHILE_DRAGGING') {
      !(state.phase === 'COLLECTING' || state.phase === 'DROP_PENDING')
        ? invariant$1(false, `Unexpected ${action.type} received in phase ${state.phase}`)
        : void 0
    ...
    if (action.type === 'MOVE') {
      ...
      !isMovementAllowed(state)
        ? invariant$1(false, `${action.type} not permitted in phase ${state.phase}`)
        : void 0
    ...
    if (action.type === 'UPDATE_DROPPABLE_IS_ENABLED') {
      ...
      !isMovementAllowed(state)
        ? invariant$1(false, `Attempting to move in an unsupported phase ${state.phase}`)
        : void 0
      const { id, isEnabled } = action.payload
      const target = state.dimensions.droppables[id]
      !target
        ? invariant$1(false, `Cannot find Droppable[id: ${id}] to toggle its enabled state`)
        : void 0
    ...
    if (action.type === 'MOVE_BY_WINDOW_SCROLL') {
      ...
      !isMovementAllowed(state)
        ? invariant$1(false, `Cannot move by window in phase ${state.phase}`)
        : void 0
      !state.isWindowScrollAllowed
        ? invariant$1(false, 'Window scrolling is currently not supported for fixed lists')
        : void 0
    ...
    if (
      ...
      !(state.phase === 'DRAGGING')
        ? invariant$1(false, `${action.type} received while not in DRAGGING phase`)
        : void 0
    ...
    if (action.type === 'DROP_PENDING') {
      ...
      !(state.phase === 'COLLECTING')
        ? invariant$1(false, 'Can only move into the DROP_PENDING phase from the COLLECTING phase')
        : void 0
      const newState = {
        ...state,
        phase: 'DROP_PENDING',
    ...
    if (action.type === 'DROP_ANIMATE') {
      ...
      !(state.phase === 'DRAGGING' || state.phase === 'DROP_PENDING')
        ? invariant$1(false, `Cannot animate drop from phase ${state.phase}`)
        : void 0
      const result = {
  ...
  var lift = (marshal) => (_ref) => {
    ...
    return (next) => (action) => {
      ...
      !(getState().phase === 'IDLE')
        ? invariant$1(false, 'Unexpected phase to start a drag')
        : void 0
      dispatch(flush())
      dispatch(
  ...
  const moveTo = (offset) =>
    isEqual$2(offset, origin) ? undefined : `translate(${offset.x}px, ${offset.y}px)`
  const transforms = {
    moveTo,
  ...
  const dropMiddleware = (_ref) => {
    ...
    return (next) => (action) => {
      ...
      !!isWaitingForDrop
        ? invariant$1(false, 'A DROP action occurred while DROP_PENDING and still waiting')
        : void 0
      !(state.phase === 'DRAGGING' || state.phase === 'DROP_PENDING')
        ? invariant$1(false, `Cannot drop in phase: ${state.phase}`)
        : void 0
      ...
      const isAnimationRequired =
        !isEqual$2(state.current.client.offset, newHomeClientOffset) || Boolean(result.combine)
  ...
  const isCriticalEqual = (first, second) => {
    ...
    const isDroppableEqual =
      first.droppable.id === second.droppable.id && first.droppable.type === second.droppable.type
  ...
  var getPublisher = (getResponders, announce) => {
    ...
    const start = (critical, mode) => {
      ...
      asyncMarshal.add(() => {
        withTimings('onDragStart', () =>
          execute(getResponders().onDragStart, data, announce, preset$1.onDragStart),
        )
      })
    }
    const update = (critical, impact) => {
      ...
      const combine = tryGetCombine(impact)
      !dragging
        ? invariant$1(false, 'Cannot fire onDragMove when onDragStart has not been called')
        : void 0
      ...
      asyncMarshal.add(() => {
        withTimings('onDragUpdate', () =>
          execute(getResponders().onDragUpdate, data, announce, preset$1.onDragUpdate),
        )
      })
    }
    const flush = () => {
    ...
    const drop = (result) => {
      !dragging
        ? invariant$1(false, 'Cannot fire onDragEnd when there is no matching onDragStart')
        : void 0
      dragging = null
  ...
  const dropAnimationFinishMiddleware = (store) => (next) => (action) => {
    ...
    !(state.phase === 'DROP_ANIMATING')
      ? invariant$1(false, 'Cannot finish a drop animating when no drop is occurring')
      : void 0
    store.dispatch(
      completeDrop({
  ...
  var autoScroll = (autoScroller) => (store) => (next) => (action) => {
    ...
    if (action.type === 'INITIAL_PUBLISH') {
      ...
      const state = store.getState()
      !(state.phase === 'DRAGGING')
        ? invariant$1(false, 'Expected phase to be DRAGGING after INITIAL_PUBLISH')
        : void 0
  ...
  var createStore = (_ref) => {
    let { dimensionMarshal, focusMarshal, styleMarshal, getResponders, announce, autoScroller } =
      _ref
  ...
  var createDimensionMarshal = (registry, callbacks) => {
    ...
    const updateDroppableScroll = (id, newScroll) => {
      ...
      !registry.droppable.exists(id)
        ? invariant$1(false, `Cannot update the scroll on Droppable ${id} as it is not registered`)
        : void 0
    ...
    const subscriber = (event) => {
      !collection
        ? invariant$1(false, 'Should only be subscribed when a collection is occurring')
        : void 0
  ...
  var createJumpScroller = (_ref) => {
    ...
    const jumpScroller = (state) => {
      ...
      !destination
        ? invariant$1(false, 'Cannot perform a jump scroll when there is no destination')
        : void 0
  ...
  function useStyleMarshal(contextId, nonce) {
    ...
    useLayoutEffect(() => {
      !(!alwaysRef.current && !dynamicRef.current)
        ? invariant$1(false, 'style elements already mounted')
        : void 0
  ...
  function createRegistry() {
    ...
    const draggableAPI = {
      ...
      getAllByType: (type) =>
        Object.values(entries.draggables).filter((entry) => entry.descriptor.type === type),
    }
    ...
    const droppableAPI = {
      ...
      getAllByType: (type) =>
        Object.values(entries.droppables).filter((entry) => entry.descriptor.type === type),
    }
    function clean() {
      entries.draggables = {}
      entries.droppables = {}
...
`
    ...
    const startPendingDrag = useCallback(
      function startPendingDrag(actions, point) {
        !(phaseRef.current.type === 'IDLE')
          ? invariant$1(false, 'Expected to move from IDLE to PENDING drag')
          : void 0
        phaseRef.current = {
          type: 'PENDING',
          point,
    ...
    const startCaptureBinding = useMemo(
      () => ({
        ...
        fn: function onKeyDown(event) {
          ...
          function stop() {
            !isCapturing
              ? invariant$1(false, 'Cannot stop capturing a keyboard drag when not capturing')
              : void 0
            isCapturing = false
            unbindEventsRef.current()
            listenForCapture()
  ...
  function useTouchSensor(api) {
    ...
    const startDragging = useCallback(
      function startDragging() {
        const phase = getPhase()
        !(phase.type === 'PENDING')
          ? invariant$1(false, `Cannot start dragging from phase ${phase.type}`)
          : void 0
      ...
      useDevSetupWarning(() => {
        !(previousRef.current.length === sensorHooks.length)
          ? 'development' !== 'production'
            ? invariant$1(false, 'Cannot change the amount of sensor hooks after mounting')
            : invariant$1(false)
          : void 0
      })
    })
  ...
  function useValidation$1(props, contextId, getRef) {
    useDevSetupWarning(() => {
      ...
      !id
        ? 'development' !== 'production'
          ? invariant$1(false, 'Draggable requires a draggableId')
          : invariant$1(false)
        : void 0
      !(typeof id === 'string')
        ? 'development' !== 'production'
      ...
      !Number.isInteger(props.index)
        ? 'development' !== 'production'
          ? invariant$1(false, `${prefix(id)} requires an integer index prop`)
          : invariant$1(false)
        : void 0
      ...
      if (props.isEnabled) {
        !findDragHandle(contextId, id)
          ? 'development' !== 'production'
            ? invariant$1(false, `${prefix(id)} Unable to find drag handle`)
            : invariant$1(false)
          : void 0
      }
    })
    ...
    useDev(() => {
      ...
      useDevSetupWarning(() => {
        !(isClone === initialRef.current)
          ? 'development' !== 'production'
            ? invariant$1(false, 'Draggable isClone prop value changed during component life')
            : invariant$1(false)
          : void 0
      }, [isClone])
    })
  ...
  function useDroppablePublisher(args) {
      ...
      () =>
        memoizeOne((x, y) => {
          !whileDraggingRef.current
            ? invariant$1(false, 'Can only update scroll when dragging')
            : void 0
          const scroll = {
            x,
            y,
    ...
    const onClosestScroll = useCallback(() => {
      ...
      !(dragging && closest)
        ? invariant$1(false, 'Could not find scroll options while scrolling')
        : void 0
    ...
    const getDimensionAndWatchScroll = useCallback(
      (windowScroll, options) => {
        !!whileDraggingRef.current
          ? invariant$1(false, 'Cannot collect a droppable while a drag is occurring')
          : void 0
  ...
  const Placeholder = (props) => {
    ...
    const [isAnimatingOpenOnMount, setIsAnimatingOpenOnMount] = React.useState(
      props.animate === 'open',
    )
    React.useEffect(() => {
      if (!isAnimatingOpenOnMount) {
  ...
  const shared = [
    ...
    function boolean(_ref2) {
      ...
      !isBoolean(props.isDropDisabled)
        ? invariant$1(false, 'isDropDisabled must be a boolean')
        : void 0
      !isBoolean(props.isCombineEnabled)
        ? invariant$1(false, 'isCombineEnabled must be a boolean')
        : void 0
      !isBoolean(props.ignoreContainerClipping)
        ? invariant$1(false, 'ignoreContainerClipping must be a boolean')
        : void 0
    },
    function ref(_ref3) {
  ...
  const virtual = [
    function hasClone(_ref5) {
      let { props } = _ref5
      !props.renderClone
        ? invariant$1(false, 'Must provide a clone render function (renderClone) for virtual lists')
        : void 0
    },
    function hasNoPlaceholder(_ref6) {
      let { getPlaceholderRef } = _ref6
      !!getPlaceholderRef()
        ? invariant$1(false, 'Expected virtual list to not have a placeholder')
        : void 0
    },
  ]
